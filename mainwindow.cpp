...
void MainWindow::updateSettings()
{
    ...
    string graphicsconf = std::format(
        ...
        "end egl display\n \n begin wfd device 1\n wfd-dlls = libwfdcfg-rpi4-generic.so "
        "libWFDrpi4-drm-atomic.so\n \n # Run \"drm-probe-displays\" to list the available displays "
        ...
        "display 1: HDMI-A\n # display 2: HDMI-A\n \n # Pipeline IDs 1 to 9 are used for DRM "
        ...
        "begin display 1\n video-mode = {} @ 60\n stack-size = 65536  # in units of bytes\n "
        "force-composition = true\n cursor = on # has to be on, otherwise it takes 10s to "
    ...
    SSHClient rpi(u, p, a);
    rpi.connect();
    rpi.sendCommand(std::format("echo \"{}\" >> /system/lib/graphics/rpi4-drm/graphics-rpi4.conf", graphicsconf));
    rpi.sendCommand(std::format("echo \"{}\" >> /boot/qnx_config.txt", cameraconf));
}
