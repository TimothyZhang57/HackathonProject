...
SSHClient::SSHClient(std::string user, std::string password, std::string host,
                     int port, int verbosity)
    : m_user(user), m_password(password), m_host(host), m_port(port),
...
bool SSHClient::sendCommand(const std::string &command) {
  try {
    ...
    std::cout << "Error during command execution: " << e.getError()
              << std::endl;
    return false;
  }
}
